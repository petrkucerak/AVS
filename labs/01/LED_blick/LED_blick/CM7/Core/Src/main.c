...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  /* USER CODE BEGIN WHILE */
   while (1) {
      ...
      uint8_t mode = BLICK_NO;

      // read input from joystick
      // JOY_RIGHT_Pin | JOY_LEFT_Pin | JOY_UP_Pin | JOY_DOWN_Pin;
      if (HAL_GPIO_ReadPin(GPIOK, JOY_RIGHT_Pin))
         mode = BLICK_RIGHT;
      else if (HAL_GPIO_ReadPin(GPIOK, JOY_LEFT_Pin))
         mode = BLICK_LEFT;
      else if (HAL_GPIO_ReadPin(GPIOK, JOY_DOWN_Pin) ||
               HAL_GPIO_ReadPin(GPIOK, JOY_UP_Pin))
         mode = BLICK_NO;
